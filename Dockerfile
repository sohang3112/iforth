...
RUN jupyter kernelspec install --user $(python -c 'import importlib.resources as r; print(r.files("forth_kernel"))')/kernelspec
...
COPY kernelspec/ kernelspec/
RUN pip install -e . --verbose
RUN jupyter kernelspec install --user kernelspec      
